...
func TestPTPTestResultExplainSuccess(t *testing.T) {
	...
	expected := `linearizability test against "server01" passed`
	require.Equal(t, expected, tr.Explain())
}
...
func TestPTPTestResultExplainError(t *testing.T) {
	...
	expected := `linearizability test against "server01" couldn't be completed because of error: test error`
	require.Equal(t, expected, tr.Explain())
}
...
func TestPTPTestResultExplainFailed(t *testing.T) {
	...
	explanation := tr.Explain()
	require.Contains(t, explanation, "failed")
	require.Contains(t, explanation, "-1µs")
}

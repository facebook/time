...
type AsymmetryConfig struct {
	...
	MaxPortChanges             uint16        `yaml:"max_port_changes"`          // rack: moves one search may spend, zero spends none
	Simple                     bool          `yaml:"simple"`                    // retained so existing configs still parse; simple is the default when rack is off
...
func DefaultConfig() *Config {
	return &Config{
		...
		Asymmetry: AsymmetryConfig{
			...
			// rack's search budget; the Go zero value would disable the search
			MaxPortChanges: 4,
		},
	}
}
